...
app.get('/auth/discord/callback', async (req, res) => {
  ...
  try {
    ...
    // Upsert user in DB — owner gets everything auto-granted
    const isOwner = discordUser.id === OWNER_ID;
    const { rows } = await pool.query(`
      INSERT INTO users (id, username, discriminator, avatar, owned_themes, is_member)
      VALUES ($1, $2, $3, $4, $5, $6)
      ON CONFLICT (id) DO UPDATE SET
        username = EXCLUDED.username,
        discriminator = EXCLUDED.discriminator,
        avatar = EXCLUDED.avatar,
        owned_themes = CASE WHEN users.id = $7 THEN $5 ELSE users.owned_themes END,
        is_member = CASE WHEN users.id = $7 THEN true ELSE users.is_member END,
        updated_at = NOW()
      RETURNING *
    `, [
      discordUser.id, discordUser.username, discordUser.discriminator || '0', discordUser.avatar,
      isOwner ? ALL_THEMES : ['yellow'],
      isOwner,
      OWNER_ID
    ]);
